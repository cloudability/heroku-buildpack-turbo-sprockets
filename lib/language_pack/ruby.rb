...
private
  ...
  def build_ruby?
    @build_ruby ||= !ruby_version_jruby? && !%w{ruby-1.9.3 ruby-2.1.1 ruby-2.1.2}.include?(ruby_version)
  end
  ...
  def build_bundler
    log("bundle") do
      ...
      if $?.success?
        ...
        pipe "bundle config --delete dry_run" # Get around a Heroku bug causing us to not ditch old versions of gems.
        pipe "bundle clean"
        cache_store ".bundle"
